...
def extract_lineage_from_sql(sql: str, dialect: str = "sqlite") -> List[Dict[str, object]]:
    ...
    `lineage_from_sql`. Unqualified column references that cannot be
    resolved to a single table are bucketed under "unknown" and dropped
    when building a Lineage object.
...
def lineage_from_sql(sql: str, filter_logic: str = "", dialect: str = "sqlite") -> Lineage:
    ...
    ready to attach to an AMU. Unresolved ("unknown") column references
    are dropped -- pass an explicit filter_logic if you need to capture
    predicate semantics sqlglot can't attribute to a table.
    """
    raw_steps = extract_lineage_from_sql(sql, dialect=dialect)
    steps = tuple(
        LineageStep(table=step["table"], columns_used=tuple(step["columns"]))
        for step in raw_steps
        if step["table"] != "unknown"
    )
    return Lineage(
        steps=steps,
